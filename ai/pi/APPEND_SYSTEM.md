...
- **Commit / PR drafting** — when the user asks for a commit message, PR title, or PR body, or says "branch and pr" and the diff is ready, you **must** call `subagent({ agent: "commit-pr-drafter", task: <diff or commit range> })`. Do not draft commits or PR bodies inline.
